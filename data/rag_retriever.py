...
BGE_EMBED_MODEL    = os.getenv("BGE_EMBED_MODEL", "BAAI/bge-small-zh-v1.5")
...
DISABLE_VECTOR_SEARCH = os.getenv("DISABLE_VECTOR_SEARCH", "false").strip().lower() in {"1", "true", "yes", "y"}
# 检索超参
...
def retrieve(
    ...
    if DISABLE_VECTOR_SEARCH:
        vector_results = []
        log.info("  向量检索已禁用，使用 BM25-only fallback")
    else:
