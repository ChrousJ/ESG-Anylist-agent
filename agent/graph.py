      ...
      evaluator_d  （数据质量检查：SQL 返回的数据对不对？RAG 召回的内容相关吗？）
          ...
          └── pass / pass_with_warnings
           ...
           synthesizer  （生成 MSCI 框架的四层分析报告）
                │
                ▼
           disclosure_scorer（披露质量确定性评分）
                │
                ▼
           greenwashing_detector（强表述-弱证据风险识别）
                │
                ▼
...
from agent.nodes.disclosure_scorer import disclosure_scorer_node  # ⑩ 披露质量评分
from agent.nodes.greenwashing_detector import greenwashing_detector_node  # ⑪ 绿漂风险识别
...
def knowledge_answer_node(state: AgentState) -> AgentState:
    ...
    if os.getenv("OFFLINE_DETERMINISTIC_MODE", "false").strip().lower() in {"1", "true", "yes", "y"}:
        if any(term in query.lower() for term in ["范围一", "范围二", "范围三", "scope 1", "scope 2", "scope 3"]):
            answer = (
                "范围一是企业自有或控制源产生的直接温室气体排放；"
                "范围二是企业购买电力、蒸汽、供热或制冷对应的间接排放；"
                "范围三是价值链中除范围二外的其他间接排放，例如采购、物流、员工通勤和产品使用。"
                "不同企业的统计边界可能不同，横向比较时需核对核算口径。"
            )
        else:
            answer = "当前为离线确定性模式，请联网配置 LLM 后获取更完整的 ESG 概念解释。"
...
def no_data_response_node(state: AgentState) -> AgentState:
    ...
    # Terminal no-data responses must not carry unrelated retrieval evidence.
    # This is a safety boundary: the answer explains the gap without citing
    # another company or leaking stale SQL/RAG state.
    state["sources"] = []
    state["sql_result"] = None
    state["sql_query_executed"] = ""
    state["rag_result"] = {"chunks": [], "coverage": {}}
...
def build_graph() -> StateGraph:
    ...
    builder.add_edge("synthesizer", "disclosure_scorer")
    builder.add_edge("disclosure_scorer", "greenwashing_detector")
    builder.add_edge("greenwashing_detector", "evaluator_o")
