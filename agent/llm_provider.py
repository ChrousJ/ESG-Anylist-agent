...
_gemini_client = None
_qwen_client = None
_openai_compatible_client = None
...
def _get_openai_compatible_client():
    """Client for any OpenAI-compatible endpoint configured via standard env vars."""
    global _openai_compatible_client
    if _openai_compatible_client is None:
        import openai
        _openai_compatible_client = openai.OpenAI(
...
def get_default_model() -> str:
    ...
    if provider in {"openai", "openai_compatible"}:
        return os.getenv("OPENAI_MAIN_MODEL", "gpt-4.1-mini")
...
def llm_generate_content(
    ...
    if provider in {"qwen", "openai", "openai_compatible"}:
        ...
        client = _get_qwen_client() if is_qwen else _get_openai_compatible_client()
        default_model = (
            os.getenv("QWEN_MAIN_MODEL", "qwen3.5-flash")
            if is_qwen else os.getenv("OPENAI_MAIN_MODEL", "gpt-4.1-mini")
        )
        model_name = (model or "").strip() or default_model
        timeout_env = "QWEN_REQUEST_TIMEOUT_SEC" if is_qwen else "OPENAI_REQUEST_TIMEOUT_SEC"
        raw_request_timeout = _get_config_attr(
            config,
            "timeout",
            os.getenv(timeout_env, "60"),
        )
        ...
        log.info(
            "%s call model=%s latency_ms=%s prompt_tokens=%s completion_tokens=%s total_tokens=%s timeout_s=%.1f max_tokens=%s thinking=%s",
            provider,
            model_name,
            latency_ms,
