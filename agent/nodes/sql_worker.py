...
def _generate_sql_deterministic(entities: dict) -> str:
    """Build a safe SELECT for known metric keys when LLM Text2SQL is unavailable."""
    ...
    if not years or not metrics or (not companies and not industry):
        return ""
    ...
    industry_tables = {
        "bank": ("esg_banking_metrics", {"green_finance_balance", "inclusive_finance_balance"}),
        "new_energy": ("esg_auto_metrics", {"scope_3_emissions", "rd_investment_total", "supplier_esg_audit_ratio"}),
        "power": ("esg_power_metrics", {"scope_3_emissions", "clean_energy_ratio", "rd_investment_total"}),
    }
    selected = []
    joins = []
    for metric in metrics:
        if metric in universal:
            selected.append(f"u.{metric} AS {metric}")
            continue
        table_info = industry_tables.get(industry)
        if table_info and metric in table_info[1]:
            alias = "i"
            selected.append(f"{alias}.{metric} AS {metric}")
            if not joins:
                joins.append(
                    f"LEFT JOIN {table_info[0]} {alias} "
                    "ON u.company_name=i.company_name AND u.year=i.year"
                )
    if not selected:
        return ""

    year_values = ", ".join(str(y) for y in years)
    filters = [f"u.year IN ({year_values})"]
    if companies:
        company_values = ", ".join("'" + c.replace("'", "''") + "'" for c in companies)
        filters.append(f"u.company_name IN ({company_values})")
    elif industry:
        filters.append("u.industry = '" + str(industry).replace("'", "''") + "'")
    return (
        "SELECT u.company_name, u.year, " + ", ".join(selected)
        + " FROM esg_universal_metrics u " + " ".join(joins)
...
def sql_worker_node(state: AgentState) -> AgentState:
    ...
    if sql_gen_result["status"] != "success":
        generated_sql = _generate_sql_deterministic(entities)
        if generated_sql:
            log.warning("LLM Text2SQL unavailable; using deterministic entity/metric SELECT fallback")
        else:
