...
def _companies_for_industry(industry: str) -> list[str]:
    mapping = _load_supported_company_industries()
    return sorted(company for company, value in mapping.items() if value == industry)

def _is_industry_wide_query(query: str, industry: str, companies: list[str]) -> bool:
    if companies or not industry:
        return False
    return any(marker in query for marker in ("所有公司", "全部公司", "全行业", "行业所有", "同行所有"))
...
def _extract_years(query: str) -> list[int]:
    ...
    if m:
        ...
        # 本项目数据覆盖范围固定为 2022--2024。
        # “近三年”应覆盖完整的三年数据窗口，而不是依赖运行机器的当前年份。
        available_years = [2022, 2023, 2024]
        inferred = available_years[-n:] if n > 0 else []
...
def _extract_metrics(query: str) -> list[str]:
    """从 query 中匹配指标关键词，并展开少量明确的复合业务意图。"""
    found = []
    q_lower = query.lower()
    for alias, key in METRIC_ALIASES.items():
        if alias.lower() in q_lower and key not in found:
            found.append(key)
    if "碳排放披露质量" in query or "温室气体披露质量" in query:
        for key in ("scope_1_emissions", "scope_2_emissions"):
            if key not in found:
                found.append(key)
    return found
...
def context_node(state: AgentState) -> AgentState:
    ...
    if need_llm and offline_mode:
        resolved_query = query
        intent = "qa"
        # 在离线模式下也保持与在线规则优先级一致：知识问题优先于
        # “指标词命中”，否则“范围一范围二是什么”会被误路由到复杂查询。
        has_knowledge_pattern = any(p.lower() in query.lower() for p in KNOWLEDGE_PATTERNS)
        if has_knowledge_pattern and not companies and not years:
            query_class = "knowledge"
            need_clarify = False
            clarify_question = ""
        else:
            query_class = rule_class or "complex"
            intent = (
                "compare" if any(x in query for x in ("对比", "比较", "排名"))
                else "trend" if any(x in query for x in ("趋势", "走势", "变化"))
                else "qa"
            )
        ...
        log.info(f"离线规则补全：class={query_class}, companies={companies}")
    elif need_llm:
        try:
    ...
    industry = _infer_industry(companies, query)
    industry_wide = _is_industry_wide_query(query, industry, companies)
    if industry_wide:
        companies = _companies_for_industry(industry)
        intent = "compare"
        log.info(f"行业全量查询展开为 {len(companies)} 家公司")
    compare_dimension = _infer_compare_dimension(companies, years, intent)
