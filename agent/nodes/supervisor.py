...
def _all_requested_metrics_missing(state: AgentState) -> bool:
    """Return True when SQL has no usable value for any requested metric.

    This is intentionally conservative: a terminal no-data response is only
    selected when every requested metric is absent/NULL and RAG has no target
    evidence. Partial results must continue through the normal synthesis path.
    """
    entities = state.get("entities", {}) or {}
    metrics = [str(m) for m in entities.get("metrics", []) if m]
    if not metrics:
        return False
    df = get_sql_result_dataframe(state)
    if df is None or len(df) == 0:
        return True
    present_columns = [m for m in metrics if m in getattr(df, "columns", [])]
    if not present_columns:
        return True
    try:
        return all(df[m].notna().sum() == 0 for m in present_columns)
    except Exception:
        return False


def _rag_has_requested_target(state: AgentState) -> bool:
    entities = state.get("entities", {}) or {}
    companies = set(str(c) for c in entities.get("companies", []) if c)
    years = set(int(y) for y in entities.get("years", []) if str(y).isdigit())
    chunks = (state.get("rag_result") or {}).get("chunks", [])
    if not chunks:
        return False
    for chunk in chunks:
        ...
        try:
            ...
            year = None
        if (not companies or company in companies) and (not years or year in years):
            return True
    return False
...
def supervisor_node(state: AgentState) -> AgentState:
    ...
    active_replan_error_type = eval_d_errors[0].get("type", "") if eval_d_errors else ""
    ...
    if is_replan:
        ...
        if terminal_response is None and error_type in {"L1_MISSING", "SQL_EMPTY"} \
                and _all_requested_metrics_missing(state) \
                and not _rag_has_requested_target(state):
            entities = state.get("entities", {}) or {}
            companies = entities.get("companies", []) or []
            metrics = entities.get("metrics", []) or []
            terminal_response = (
                "coverage_gap" if entities.get("unsupported_companies") and not entities.get("supported_companies") else "not_disclosed",
                f"当前请求的公司/指标没有可验证值：{'、'.join(companies) or '行业范围'} / {'、'.join(metrics)}。",
            )
        ...
        elif error_type == "RAG_TARGET_COVERAGE_MISSING":
            missing = replan_error.get("missing_targets", [])
            reason = f"RAG_TARGET_COVERAGE_MISSING → 对缺失公司-年份定向补检索 {missing}"
            state["retry_count"] = increment_retry(state, "rag")
            log.info(reason)
    ...
    # ── 决定数据源策略 ───────────────────────────────────────────────────────
    replan_error_for_strategy = {"type": active_replan_error_type} if active_replan_error_type else None
    strategy = _decide_strategy(entities, replan_error_for_strategy)

    # Re-plan 时强制使用调整后的策略
    if is_replan:
        error_type = active_replan_error_type
        if error_type in ("SQL_EMPTY", "L1_MISSING"):
            strategy = "rag_only"
        elif error_type in ("RAG_LOW_RECALL", "RAG_TARGET_COVERAGE_MISSING"):
            strategy = "parallel"
        elif error_type == "RAG_LOW_RELEVANCE":
            strategy = "sql_only"
    ...
    # ── 组装执行计划 ─────────────────────────────────────────────────────────
    replan_type_for_rag = active_replan_error_type
    ...
    elif replan_type_for_rag == "RAG_TARGET_COVERAGE_MISSING":
        rag_strategy = "targeted_recovery"
