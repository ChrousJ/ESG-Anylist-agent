...
class ChatResponse(BaseModel):
    ...
    disclosure_quality: Optional[dict] = Field(default=None, description="Deterministic disclosure-quality score and risk flags.")
    greenwashing_risks: Optional[dict] = Field(default=None, description="Rule-based claim-evidence mismatch risk radar.")

    query_class: str = Field(description="knowledge/complex/clarify")
    entities: dict = Field(default_factory=dict, description="Parsed entities.")
    industry: str = Field(default="", description="Industry label.")
    sources: list[SourceItem] = Field(default_factory=list, description="Citations.")
...
app = FastAPI(
    title="ESG-Insight Agent API",
    description="Evaluable LangGraph ESG analysis agent with SQL + RAG evidence and dual evaluators",
    version="1.1.0",
    lifespan=lifespan,
)
...
@app.get("/api/capabilities", summary="Project capabilities and evidence coverage")
def capabilities():
...
def _build_response(final_state: dict, latency_ms: int) -> ChatResponse:
    ...
    return ChatResponse(
        ...
        disclosure_quality=final_state.get("disclosure_quality"),
        greenwashing_risks=final_state.get("greenwashing_risks"),
...
async def _stream_generator(body: ChatRequest, request_id: str) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        complete_event = {
            ...
            "chart_spec": response.chart_spec.model_dump() if response.chart_spec else None,
            "disclosure_quality": response.disclosure_quality,
            "greenwashing_risks": response.greenwashing_risks,
            "eval_o_status": response.eval_o_status,
            "is_degraded": response.is_degraded,
            "missing_summary": response.missing_summary,
            "sources": [s.model_dump() for s in response.sources[:5]],
            "conversation_id": conversation_id,
            "trace_id": trace_id,
